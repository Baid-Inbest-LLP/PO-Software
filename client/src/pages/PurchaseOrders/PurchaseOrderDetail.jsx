...
const PurchaseOrderDetail = () => {
  ...
  const CONFIRM_CONFIG = {
    ...
    approved_by_admin: { title: 'PO Admin approval', message: 'Approve this purchase order and send it to Superadmin for final approval?', confirmLabel: 'Approve', variant: 'primary' },
    completed: { title: 'Final approval', message: 'Finalize and mark this PO as completed? It can no longer be edited; downloads are available after approval.', confirmLabel: 'Final approve', variant: 'primary' },
  };
  ...
  const totals = lineItems.reduce((acc, item) => {
    ...
    return {
      qty: acc.qty + qty,
      preGst: acc.preGst + calcPreGstAmount(item),
  ...
  const roundedItemsTotal = Math.round(totals.total);
  const roundedGrandTotal = Math.round(order.totalAmount || totals.total);
  const formatCurrencyDisplay = (amount) => formatCurrency(amount).replace(/\.00$/, '');
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
            <div>
              ...
              <div className="mt-1 border-l-2 border-primary-200 pl-3 space-y-1.5 max-w-2xl">
                <p className="text-sm text-gray-600 leading-relaxed">
                  <span className="font-semibold text-gray-800">Created</span>
                  {' '}at <span className="text-gray-800 font-medium">{formatDate(order.createdAt)}</span>
                  {' '}by <span className="text-gray-800 font-medium">{order.createdBy?.name || order.createdBy?.email || '—'}</span>
                </p>
                {order.approvedByAdminAt && (
                  <p className="text-sm text-gray-600 leading-relaxed">
                    <span className="font-semibold text-gray-800">Approved</span>
                    {' '}at <span className="text-gray-800 font-medium">{formatDate(order.approvedByAdminAt)}</span>
                    {' '}by <span className="text-gray-800 font-medium">{order.approvedByAdmin?.name || order.approvedByAdmin?.email || '—'}</span>
                  </p>
                )}
                {order.status === 'completed' && order.completedAt && (
                  <p className="text-sm text-gray-600 leading-relaxed">
                    <span className="font-semibold text-gray-800">Final approval</span>
                    {' '}at <span className="text-gray-800 font-medium">{formatDate(order.completedAt)}</span>
                    {' '}by <span className="text-gray-800 font-medium">{order.completedBy?.name || order.completedBy?.email || '—'}</span>
                  </p>
                )}
              </div>
            </div>
            ...
            <tbody>
              {lineItems.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td className="text-right">{formatCurrencyDisplay(item.unitPrice)}</td>
                  <td className="text-right text-gray-500">{item.discount > 0 ? `${item.discount}%` : ''}</td>
                  <td className="text-right text-red-700 font-medium">{calcDiscountAmt(item) > 0 ? formatCurrencyDisplay(Math.round(calcDiscountAmt(item))) : ''}</td>
                  <td className="text-right font-semibold text-slate-700">{formatCurrencyDisplay(Math.round(calcPreGstAmount(item)))}</td>
                  <td className="text-right text-gray-500">{item.gstRate > 0 ? `${item.gstRate}%` : ''}</td>
                  <td className="text-right text-emerald-700 font-medium">{calcGstAmt(item) > 0 ? formatCurrencyDisplay(Math.round(calcGstAmt(item))) : ''}</td>
                  <td className="text-right font-bold text-gray-900">{formatCurrencyDisplay(Math.round(Number(item.totalPrice) || 0))}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t-2 border-gray-200 bg-blue-50">
                ...
                <td className="text-right font-bold text-gray-800 align-middle text-base py-2 px-4">{totals.qty}</td>
                <td className="text-center text-gray-400 align-middle text-base py-2 px-4"></td>
                <td className="text-right text-gray-400 align-middle text-base py-2 px-4"></td>
                <td className="text-right text-gray-400 align-middle text-base py-2 px-4"></td>
                <td className="text-right font-bold text-red-700 align-middle text-base py-2 px-4">{formatCurrencyDisplay(Math.round(totals.discount))}</td>
                <td className="text-right font-bold text-slate-700 align-middle text-base py-2 px-4">{formatCurrencyDisplay(Math.round(totals.preGst))}</td>
                <td className="text-right text-gray-400 align-middle text-base py-3 px-4"></td>
                <td className="text-right font-bold text-emerald-700 align-middle text-base py-2 px-4">{formatCurrencyDisplay(roundedGstTotal)}</td>
                <td className="text-right font-bold text-primary-700 align-middle text-lg py-2 px-4">{formatCurrencyDisplay(roundedItemsTotal)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
          ...
          <div className="flex flex-col lg:flex-row items-end justify-between gap-6">
            ...
            <div className="w-80 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-gray-800 text-base">{formatCurrencyDisplay(Math.round(order.subtotal || 0))}</span>
              </div>
              {order.shippingCost > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 font-medium">Shipping</span>
                  <span className="font-bold text-gray-800 text-base">{formatCurrencyDisplay(order.shippingCost)}</span>
                </div>
              )}
              <div className="border-t-2 border-gray-900 pt-3 flex justify-between items-baseline">
                <span className="font-bold text-gray-900 text-lg">Grand Total</span>
                <span className="text-2xl font-bold text-primary-700 tracking-tight">{formatCurrencyDisplay(roundedGrandTotal)}</span>
              </div>
            </div>
          </div>
        </div>
